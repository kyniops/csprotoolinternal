...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader (precompiled)
    {
        if (bd->pd3dDevice->CreateVertexShader(g_pVertexShaderBlob, sizeof(g_pVertexShaderBlob), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_pVertexShaderBlob, sizeof(g_pVertexShaderBlob), &bd->pInputLayout) != S_OK)
            return false;
    ...
    // Create the pixel shader (precompiled)
    {
        if (bd->pd3dDevice->CreatePixelShader(g_pPixelShaderBlob, sizeof(g_pPixelShaderBlob), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
